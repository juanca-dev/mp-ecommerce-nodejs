...
const preferencia = {
  ...
  notification_url: "",
  external_reference: "juancarlos17.mf@gmail.com",
};
...
app.get("/detail", async function (req, res) {
  ...
  //ahora modificaremos la back_url para indicar el dominio de  nuestra aplicación
  preferencia.back_urls.failure= `${req.get("host")}/failure`; // http://127.0.0.1:500
  preferencia.back_urls.pending= `${req.get("host")}/pending`;
  preferencia.back_urls.success= `${req.get("host")}/success`;
  //el notificiation_url solo se puede usar en ambientes de produccion (no localhost ni 127.0.0.1)
  // porque es a ese endpoint en el cual se mandara el estado de la pasarela de pago y por ende al identificar uno de los 
  //dominios anteriores lanzara un error y no se procedera con la pasarela
